...
{
    return free_heads[select_bin_for_size(size)];
}
...
/*
 * Select the appropriate bin based on the payload size
 */
...
mem_block_header_t *find(size_t payload_size)
{
    ...
    if (payload_size == 0)
        return NULL;
    int index = select_bin_for_size(payload_size);
...
void add_to_free_list(mem_block_header_t *block)
{
    ...
    int bin_index = select_bin_for_size(size);
    // adds the block to the free list
    block->next = free_heads[bin_index];
    free_heads[bin_index] = block;
}

mem_block_header_t *combine(mem_block_header_t* p, mem_block_header_t* n, mem_block_header_t* block)
{
